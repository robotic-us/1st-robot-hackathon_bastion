...
def set_current_as_zero(client: PcmStudioClient, axis: int,
    ...
    while time.monotonic() < deadline:
        try:
            ...
            # Flash persistence can temporarily suppress Studio SDO replies.
            # The write request itself was acknowledged, so keep polling until
            # the bounded apply deadline instead of abandoning a potentially
            # successful zero update after one per-request timeout.
            continue
...
def wait_for_rebase_window(client: PcmStudioClient,
    ...
    while time.monotonic() < deadline:
        ...
        if (
            ...
            and last_status["write_window_open"]
        ):
            return last_status
        time.sleep(0.15)
...
def reload_motion_catalog(client: PcmStudioClient, slot: int,
    ...
    while time.monotonic() < deadline:
        try:
            ...
            # Catalog rescan temporarily owns the SD and can suppress Studio
            # SDO replies. Keep polling inside the bounded reload deadline.
            continue
