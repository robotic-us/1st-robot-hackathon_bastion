...
class LinuxPcmHost:
    ...
    def prepare_live_session(self) -> None:
        # A previous invocation may have ended immediately after a slot, when
        # this PCM revision no longer answers Studio SDO.  Always begin from a
        # fresh data session so repeated function calls remain deterministic.
...
def run_first_slot_stage(
    ...
    *,
    ...
    return {"arm": arm, "slot": first, "cleanup": cleanup}
...
def run_rebase_arm_second_slot_stage(
    ...
    *,
    ...
    cleanup = stop_and_servo_off(client, on_event=on_event)
    return {"origin": origin, "arm": arm, "slot": second, "cleanup": cleanup}
    ...
    while True:
        try:
            ...
            retryable = isinstance(exc, TimeoutError) or (
                "attachable storage mode (mode=1)" in str(exc)
            )
